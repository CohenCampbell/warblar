...
@app.route('/users/add_like/<int:id>', methods=["GET", "POST"])
def add_like(id):
    if not g.user:
        flash("Access unauthorized.", "danger")
        return redirect("/")
    ...
    if(Likes.query.filter(Likes.user_id==user.id, Likes.message_id==id).first()):
        db.session.delete(Likes.query.filter(Likes.user_id==user.id, Likes.message_id==id).first())
    else:
        new_like = Likes(user_id=user.id, message_id=id)
        db.session.add(new_like)
        
    db.session.commit()
    print(request.form.get("from-profile"))
    if(request.form.get("from-profile") == "True"):
